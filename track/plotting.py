...
Il grafico centrale e' `plot_track()`: la mappa. Asse verticale = RS Score,
asse orizzontale = colonna settoriale. Il momentum NON e' un secondo asse: e'
la pendenza della traiettoria, cioe' il movimento verticale del titolo nel tempo.
"""
...
POS = "#3f9e6a"   # RS Slope positivo
NEG = "#c1444b"   # RS Slope negativo
...
def _lane_positions(snapshot: pd.DataFrame, sectors_order: list[str]) -> pd.Series:
    """Coordinata x stabile: colonna settoriale del settore + scostamento deterministico.

    Lo scostamento dipende dall'ordine alfabetico del ticker dentro il settore,
    non da F: cosi' resta costante nel tempo e le traiettorie risultano verticali,
    che e' esattamente il senso di un asse verticale unico.
    """
...
def plot_track(
    ...
    *,
    ...
    x_mode:   'sector' -> 11 colonne settoriali GICS ; 'velocity' -> x = RS Slope
    """
    ...
    if snap.empty:
        _base_layout(fig, height, title or "Monitor di Forza Relativa")
    ...
    if edges.size:
        for i in range(n_bands):
            ...
            label = band_names[i] if i < len(band_names) else f"Q{i+1}"
            fig.add_hrect(
    ...
    # --- separatori di colonna settoriale ---------------------------------------------
    if x_mode == "sector":
    ...
    if trails is not None and not trails.empty:
        ...
        if xs:
            fig.add_trace(go.Scattergl(
                x=xs, y=ys, mode="lines", name="Traiettoria",
    ...
    fig.add_trace(go.Scattergl(
        ...
        marker=dict(
            ...
            colorbar=dict(title=dict(text="RS Slope", side="right"), thickness=12, len=0.45, y=0.22),
        ),
        ...
        hovertemplate=(
            ...
            "RS Score: %{customdata[3]}<br>"
            "RS Slope: %{customdata[4]}<br>"
            "Quintile: %{customdata[5]}<extra></extra>"
        ),
        showlegend=False,
    ))
    ...
    fig.update_yaxes(title="RS Score  (z-score trasversale)", range=[y_lo, y_hi])
